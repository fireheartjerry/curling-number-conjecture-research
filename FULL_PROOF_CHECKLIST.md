...
- [x] Build a definition-first two-branch bridge census through \(q\le25\)
      from the standalone seed \(G\_{\rm loc}=X^3U\), pin sharp local replays,
...
- [ ] **Boundary \(p>q\) wall for \(\mathcal J\).**
      ...
      Progress 2026-07-28 (`research/pgtq_boundary_small_r.md`): the bridge
      word is classified synthetically for every \(q\) — \(r\le3\) is
      impossible, \(r=4\) forces \(B=\texttt{2232}\), \(r=5\) has exactly
      ...
      Progress 2026-07-29 (`research/pgtq_r4_shallow_zoo.md`): the
      shallow zoo is empty (complete finite verification of all
